...
2. 上記のrakeタスクを`schedule.rb(whenever)`に1時間毎に定期実行させる(バッチ処理)設定を記述。(まだcronの編集をしただけ)
...
一度パラメーターを上書き、カラムを変更、saveにすれば良い。  
updateは上書き、saveを一括で処理してしまうため、途中でカラムの内容を変更したいときには使えない。

### find_each

- レコードを1000件(デフォルト)ずつ取得、1件ずつ処理していく
